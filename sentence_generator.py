...
#Optional function to expand the optional terminals
def expand_optional_terminals(tokens):
    ...
    for each in tokens:
        if ('(' and ')') in each:
            ...
            for j in range(len(terminals)):
                i+=1
                terminals[i] = terminals[j].copy()
                terminals[i].append(new_str2)
            
        else:
            for j in range(len(terminals)):
                terminals[j].append(each)
    return(terminals)
...
def get_hash_table(grammar_filename):
    ...
    for line in grammar_file:
    	...
    	if line:
            ...
            tokens.pop(0)
            terminals = expand_optional_terminals(tokens)
            #terminals = tokens
            for i in range(len(terminals)):
                hash_table[non_terminals].append(terminals[i])  #Updating the hash table for each entry
...
def rand_sentence_generator(hash_table, cum_freq, key):
    ...
    if symbols and non_terminals_counter > 0:   #Check for the non-terminal expansion counter and proceed if the limit is not reached yet
        ...
        if verboseOption:
            sentence = sentence.strip('\n')       #Remove any previous newline and tab characters trailing in the sentence for verbose printing
...
for _ in range(num_rand_sentences):             #Call the generator x times, look up the function parse_args to find where x came from
    ...
    print(sentence + "\n")   #Print the generated sentence on the console
